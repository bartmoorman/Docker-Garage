  ...
  <body class='h-100'>
    ...
    <script>
      $(document).ready(function() {
        $('#pincode').pincodeInput({inputs:6, hidedigits:false, complete:function(value, e, errorElement) {
          $.getJSON('src/user.php', {"action": "create", "pincode": value, "first_name": $('#first_name').val(), "last_name": $('#last_name').val(), "role": "admin"})
            .done(function(data) {
              if (data.success) {
                location.href = 'login.php';
              } else {
                $(errorElement).html('Something went wrong!');
              }
            })
